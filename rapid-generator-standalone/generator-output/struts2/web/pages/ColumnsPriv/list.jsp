...
<rapid:override name="content">
	<form id="queryForm" name="queryForm" action="<c:url value="/pages/ColumnsPriv/list.do"/>" method="get" style="display: inline;">
	<div class="queryPanel">
		...
		<div class="handleControl">
			<input type="submit" class="stdButton" style="width:80px" value="查询" onclick="getReferenceForm(this).action='${ctx}/pages/ColumnsPriv/list.do'"/>
			<input type="submit" class="stdButton" style="width:80px" value="新增" onclick="getReferenceForm(this).action='${ctx}/pages/ColumnsPriv/create.do'"/>
			<input type="button" class="stdButton" style="width:80px" value="删除" onclick="batchDelete('${ctx}/pages/ColumnsPriv/delete.do','items',document.forms.queryForm)"/>
		<div>
	</div>
		...
		<table width="100%"  border="0" cellspacing="0" class="gridBody">
		  ...
		  <tbody>
			  ...
			  <tr class="${status.count % 2 == 0 ? 'odd' : 'even'}">
				...
				<td><input type="checkbox" name="items" value="host=${item.id.host}&db=${item.id.db}&user=${item.id.user}&tableName=${item.id.tableName}&columnName=${item.id.columnName}&"></td>
				...
				<td>
					<a href="${ctx}/pages/ColumnsPriv/show.do?host=${item.id.host}&db=${item.id.db}&user=${item.id.user}&tableName=${item.id.tableName}&columnName=${item.id.columnName}&">查看</a>&nbsp;&nbsp;&nbsp;
					<a href="${ctx}/pages/ColumnsPriv/edit.do?host=${item.id.host}&db=${item.id.db}&user=${item.id.user}&tableName=${item.id.tableName}&columnName=${item.id.columnName}&">修改</a>
				</td>
			  </tr>
