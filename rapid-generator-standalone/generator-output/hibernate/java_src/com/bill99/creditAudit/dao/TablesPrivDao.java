...
@Repository
public class TablesPrivDao extends BaseHibernateDao<TablesPriv,java.lang.String>{

	public Class getEntityClass() {
		return TablesPriv.class;
	}
	...
	public Page findPage(TablesPrivQuery query) {
		...
		String sql = "select t from TablesPriv t where 1=1 "
			  	+ "/~ and t.grantor = {grantor} ~/"
				+ "/~ and t.timestamp >= {timestampBegin} ~/"
				+ "/~ and t.timestamp <= {timestampEnd} ~/"
			  	+ "/~ and t.tablePriv = {tablePriv} ~/"
			  	+ "/~ and t.columnPriv = {columnPriv} ~/"
				+ "/~ order by [sortColumns] ~/";

        //生成sql2的原因是为了不喜欢使用xsqlbuilder的同学，请修改生成器模板，删除本段的生成
        StringBuilder sql2 = new StringBuilder("select t from TablesPriv t where 1=1 ");
        ...
        if(isNotEmpty(query.getGrantor())) {
            sql2.append(" and  t.grantor = :grantor ");
        }
        ...
        if(isNotEmpty(query.getTablePriv())) {
            sql2.append(" and  t.tablePriv = :tablePriv ");
        }
